...
Use this skill to keep the agent on extraction and interpretation while Lattice handles the combinatorics.
...
To install or refresh a copy of this skill from the installed package:

```bash
lattice agent install-skill /path/to/harness/skills/lattice-workflow
```

For harnesses that don't use skill directories, copy the output of:

```bash
lattice agent instructions
```

into the harness memory, system instruction, rule file, or project instruction mechanism.
